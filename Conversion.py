...
class Conversion:
    ...
    def format_overall_rating(self):
        ...
        overall_gaps = []
        for i in range(len(self.app.students_overall) - 1):
            gap = round(((self.app.students_overall[i][1] - self.app.students_overall[i + 1][1]) / self.app.students_overall[i + 1][1]) * 100)
            overall_gaps.append(gap)
        overall_gaps.append(0)  # No gap for the last student
